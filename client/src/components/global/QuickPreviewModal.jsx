...
export default function QuickPreviewModal({ owner, repo, issueNumber, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end" onClick={onClose}
      ...
      >
        {/* Header */}
        <div className="sticky top-0 flex items-center justify-between px-5 py-4 border-b border-white/[0.06]"
          style={{ background: 'rgba(12,12,14,0.9)', backdropFilter: 'blur(20px)' }}>
          <p className="text-xs text-ink-tertiary font-mono">{owner}/{repo}</p>
        ...
        {isLoading ? (
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {issue.difficultyTag && (
                <span className={`tag text-xs ${DIFFICULTY_TAG_COLORS[issue.difficultyTag]}`}>
                  {issue.difficultyTag}
                </span>
              )}
            </div>

            <h2 className="text-base font-semibold text-ink-primary leading-snug">{issue.title}</h2>
            ...
            {issue.repo && (
              <div className="glass-sm p-4 space-y-2">
                <p className="text-xs font-semibold text-ink-secondary">{issue.repo.fullName}</p>
                <div className="flex items-center gap-4 text-xs text-ink-tertiary">
                  <span className="flex items-center gap-1"><Star size={11} />{formatNumber(issue.repo.stars)}</span>
                  <span className="flex items-center gap-1"><GitFork size={11} />{formatNumber(issue.repo.forks)}</span>
                  {issue.repo.language && <span className="text-apple-blue">{issue.repo.language}</span>}
                </div>
              </div>
            )}

            <p className="text-xs text-ink-quaternary">Updated {timeAgo(issue.updatedAt)}</p>

            {/* Actions */}
            <div className="flex gap-2 pt-1">
              {!issue.isSaved ? (
                <button onClick={handleSave} disabled={saveMutation.isPending} className="btn-primary flex-1 text-xs py-2">
                  <Bookmark size={13} />
                  {saveMutation.isPending ? 'Saving...' : 'Save Issue'}
                </button>
              ) : (
                <div className="flex items-center gap-1.5 flex-1 px-3 py-2 rounded-xl bg-apple-green/10 text-apple-green text-xs font-medium">
                  <BookmarkCheck size={13} /> Saved
                </div>
              )}
              <a href={issue.url} target="_blank" rel="noopener noreferrer" className="btn-secondary text-xs py-2 px-3">
                ...
                <ol className="space-y-2">
                  {issue.whatToDoNext.slice(0, 4).map((step, i) => (
                    <li key={i} className="flex gap-2.5 text-xs text-ink-secondary">
                      <span className="w-4 h-4 rounded-full bg-apple-blue/15 text-apple-blue flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5">
                        {i + 1}
                      </span>
                      {step}
                    </li>
