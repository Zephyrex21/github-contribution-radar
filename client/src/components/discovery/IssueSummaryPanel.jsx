...
import { Sparkles, Clock, Zap, CheckCircle2, ChevronDown, ChevronUp, Loader2, AlertCircle } from 'lucide-react';
...
export default function IssueSummaryPanel({ issue, repoFullName }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (res) => { setSummary(res.data); setCollapsed(false); },
  });

  if (!summary && !mutation.isPending && !mutation.isError) {
    return (
        ...
        <div className="p-5 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs mb-4" style={{ color: 'var(--c-text-3)' }}>
              Get a plain-English breakdown — what the issue needs, required skills, complexity, and a suggested approach. Powered by Gemini.
            </p>
      ...
      <div className="flex items-center justify-between px-5 py-3 cursor-pointer select-none"
        ...
        <div className="flex items-center gap-3">
          <button onClick={e => { e.stopPropagation(); setSummary(null); mutation.mutate(); }}
            ...
            onMouseLeave={e => e.currentTarget.style.color = 'var(--c-text-4)'}>Regenerate</button>
              ...
              <div className="flex flex-wrap gap-1.5">
                {s.skills.map(skill => (
                  <span key={skill} className="tag text-xs"
                    style={{ background: 'rgba(10,132,255,0.10)', color: 'var(--c-accent)' }}>{skill}</span>
                ))}
              </div>
            </div>
          )}
