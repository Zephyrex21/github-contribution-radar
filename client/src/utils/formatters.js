...
// Bug fix: replaced bg-white/[0.05] and bg-white/[0.06] with CSS-var-based classes
// bg-fill-tertiary = var(--c-fill-3) — adapts in light mode
export function labelColors(l) {
  ...
  // fallback: use CSS-var fill so it adapts to light mode
  return 'bg-fill-tertiary text-ink-tertiary';
}

// Bug fix: 'Saved' status used bg-white/[0.07] which is invisible in light mode
export function statusColors(status) {
  const map = {
    Saved:         'bg-fill-secondary text-ink-tertiary',   // ← was bg-white/[0.07]
  ...
  return map[status] || 'bg-fill-tertiary text-ink-tertiary';
}
